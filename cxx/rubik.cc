...
Cube::Cube(__m128i edges, __m128i corners) : edges(edges), corners(corners) {
  sanityCheck();
}
...
Cube Cube::apply(const Cube &other) const {
  ...
  out_edges = _mm_xor_si128(
      out_edges, _mm_and_si128(other.edges, _mm_set1_epi8(kEdgeAlignMask)));

  auto corner_perm =
      _mm_and_si128(other.corners, _mm_set1_epi8(kCornerPermMask));
  auto out_corners = _mm_shuffle_epi8(corners, corner_perm);
  out_corners =
      _mm_add_epi8(out_corners, _mm_and_si128(other.corners,
...
Cube Cube::invert() const {
  ...
  oe.mm = _mm_or_si128(oe.mm, _mm_and_si128(_mm_shuffle_epi8(edges, oe.mm),
...
bool Cube::operator==(const Cube &rhs) const {
  return _mm_test_all_zeros(
      _mm_or_si128(_mm_xor_si128(edges, rhs.edges),
                   _mm_and_si128(_mm_set_epi32(0, 0, 0xffffffff, 0xffffffff),
...
class facelet_parser {
  ...
  const vector<pair<int, int>> edge_indexes{
      {24, 23}, {13, 7}, {26, 27}, {37, 46},

      {3, 10},  {16, 5}, {40, 50}, {48, 34},

      {32, 21}, {19, 1}, {30, 29}, {43, 52},
  };

  const vector<tuple<int, int, int>> corner_indexes{
      {12, 11, 6}, {14, 8, 15}, {38, 39, 47}, {36, 45, 35},

      {20, 0, 9},  {18, 17, 2}, {42, 53, 41}, {44, 33, 51},
  };

  const vector<pair<int, Color>> centers{
      {4, Color::White}, {22, Color::Green},  {25, Color::Red},
      {28, Color::Blue}, {31, Color::Orange}, {49, Color::Yellow},
  };
  ...
  Result<Cube, Error> parse(const std::string &str) {
    ...
    for (auto c : centers) {
      if ((Color)str[c.first] != c.second) {
        return Error{absl::StrCat("Wrong ordering: side ", i, " should be `",
    ...
    for (auto e : edge_indexes) {
      ...
      if (fnd == edge_map.end()) {
        return Error{absl::StrCat("Can't find edge: ", string(&str[e.first], 1),
                                  "/", string(&str[e.second], 1), " (index ",
                                  e.first, "/", e.second, ")")};
      }
      eu.arr[i++] = fnd->second;
    }
    ...
    for (auto c : corner_indexes) {
      auto cs = make_tuple((Color)str[get<0>(c)], (Color)str[get<1>(c)],
                           (Color)str[get<2>(c)]);
      auto fnd = corner_map.find(cs);
      if (fnd == corner_map.end()) {
        return Error{absl::StrCat(
            "Can't find corner: ", string(&str[get<0>(c)], 1), "/",
            string(&str[get<1>(c)], 1), "/", string(&str[get<2>(c)], 1),
            " (index ", get<0>(c), "/", get<1>(c), "/", get<2>(c), ")")};
      }
      cu.arr[i++] = fnd->second;
    }
